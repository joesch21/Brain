...
Write-Host "Base=$Base Airport=$Airport Airline=$Airline DateHasData=$DateHasData DateMaybeEmpty=$DateMaybeEmpty" -ForegroundColor DarkGray

# ---------- Negative tests: airport required ----------
$sc = Get-StatusCode "$Base/api/flights?date=$DateHasData&airline=$Airline"
Assert-True ($sc -eq 400) "GET /api/flights without airport returns 400 (got $sc)"

$sc = Get-StatusCode "$Base/api/runs?date=$DateHasData&airline=$Airline&shift=ALL"
Assert-True ($sc -eq 400) "GET /api/runs without airport returns 400 (got $sc)"

$bodyMissingAirport = @{ date=$DateHasData; airline=$Airline } | ConvertTo-Json
...
$sc = Get-StatusCode "$Base/api/runs/daily?date=$DateHasData&airport=$Airport&airline=$Airline&shift=ALL"
Assert-True ($sc -eq 410) "GET /api/runs/daily returns 410 deprecated (got $sc)"

# ---------- Positive: flights read for known-good date ----------
$fl = Invoke-RestMethod "$Base/api/flights?date=$DateHasData&airport=$Airport&airline=$Airline" -TimeoutSec $TimeoutSec
...
$rn = Invoke-RestMethod "$Base/api/runs?date=$DateHasData&airport=$Airport&airline=$Airline&shift=ALL" -TimeoutSec $TimeoutSec
...
$before = Invoke-RestMethod "$Base/api/flights?date=$DateMaybeEmpty&airport=$Airport&airline=$Airline" -TimeoutSec $TimeoutSec
...
Require-Keys $pull @("ok","airport","local_date","airline","source","upstream","payload") "Pull($DateMaybeEmpty)"
...
$after = Invoke-RestMethod "$Base/api/flights?date=$DateMaybeEmpty&airport=$Airport&airline=$Airline" -TimeoutSec $TimeoutSec
